  ...
  <div class="app-container">
    <div class="search-bar">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true" label-width="80px">
        ...
        <el-form-item prop="readState" label="状态">
          <el-select v-model="queryParams.readState" placeholder="请选择" clearable>
    ...
    <el-card shadow="never" class="table-container">
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-tag v-if="row.readState" type="info">已读</el-tag>
    ...
    <ReceiverMsgInfoView ref="viewFormRef" :default-institute="defaultInstitute" />
  </div>
</template>
...
import MessageAPI, { MsgInfoListParam, MsgInfoViewDto } from "@/api/message";
import { InstituteInfoDto } from "@/api/institute";
import DatePicker from "@/components/DatePicker/index.vue";
import DictTag from "@/components/DictTag/index.vue";
import ReceiverMsgInfoView from "./components/ReceiverMsgInfoView.vue";
...
function _handleCreateDateRangeChange(_val: [Date, Date]) {
...
const tableData = ref<MsgInfoViewDto[]>([]);
...
function handleQuery() {
  ...
  MessageAPI.listMyReceiveMessageByPage(queryParams)
    .then((data) => {
 ...
 * 展示：详情窗口
 */
const viewFormRef = ref(ReceiverMsgInfoView);
function handleOpenInfoViewDrawer(row: MsgInfoViewDto) {
  // console.log("handleOpenInfoViewDrawer", row);
  MessageAPI.viewMessage({ id: row.messageId }).then((data) => {
 ...
 * 操作：删除
 */
function handleDelete(row: MsgInfoViewDto) {
  // console.log("handleDelete", row);
  MessageAPI.deleteMessageOne({ messageId: row.messageId }).then((_data) => {
