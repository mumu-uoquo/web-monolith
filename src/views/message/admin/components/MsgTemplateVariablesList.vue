<template>
  <div v-loading="props.loading" style="width: 100%">
    <el-table :data="model" border :height="height" :max-height="maxHeight" size="small">
      ...
      <el-table-column label="变量" prop="name" width="200" align="center">
      ...
      <el-table-column label="说明" prop="description" min-width="120">
...
import { MsgTemplateVariable } from "@/api/message";

/* ***************************** 参数定义 ********************************* */
// 父级组件绑定的v-modle
const model = defineModel({
  type: Array as () => MsgTemplateVariable[],
  default: () => [],
});
// 暴露给父级的自定义事件
 ...
 */
function onAddItem() {
  const temp: MsgTemplateVariable = {
    name: "",
    description: "",
  };
 ...
 * 删除
 */
function handleDelete(index: number, _row: MsgTemplateVariable) {
