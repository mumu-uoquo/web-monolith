...
/* ***************************** 表单操作 ********************************* */
// 扩展部分属性
type InfoFormParam = MsgInfoParam;
// 初始表单数据
...
const handleInfoSubmit = useDebounceFn(async () => {
  ...
  infoFormRef.value.validate((isValid: boolean) => {
    ...
    if (isEditMode.value) {
      AdminMessageAPI.updateMessage(infoFormData.value).then((_data) => {
      ...
      AdminMessageAPI.addMessage(infoFormData.value).then((_data) => {
        ElMessage.success("保存成功");
        editorRef.value.clearNotUseFile();
        editorRef.value.clearContent();
        attachmentRef.value.clear(false);
...
function loadReceiverList() {
  ...
  AdminMessageAPI.searchReceiverByRange(param).then((data) => {
