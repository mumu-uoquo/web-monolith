    ...
    <el-card shadow="never" class="table-container">
      ...
      >
        ...
        <el-table-column
          ...
          width="160"
          align="center"
          :formatter="(row) => formatDate(row.pushTime, 'YYYY-MM-DD HH:mm:ss')"
        />
        <el-table-column prop="receiverName" label="收件人" width="120" align="center" />
        <el-table-column prop="pushWay" label="方式" width="100" align="center">
          <template #default="{ row }">
            <DictTag :code="row.pushWay" />
          </template>
        </el-table-column>
        <el-table-column prop="retryCount" label="次数" width="60" align="center" />
        <el-table-column prop="pushStatus" label="状态" width="100" align="center">
          <template #default="{ row }">
            <DictTag :code="row.pushStatus" />
          </template>
        </el-table-column>
        <el-table-column prop="pushResult" label="描述" show-overflow-tooltip />
      </el-table>
...
import { MsgInfoListParam, MsgPushLogDto, PageResultMsgPushLogDto } from "@/api/message";
import DictTag from "@/components/DictTag/index.vue";

/* ***************************** 参数定义 ********************************* */
// 暴露给父级的自定义事件
const emits = defineEmits(["on-query"]);
// 暴露给父级的自定义属性
const props = defineProps({
  ...
  tableData: {
    type: Object as PropType<PageResultMsgPushLogDto>,
    default: () => ({
      total: 0,
      result: [],
    }),
...
const tableDataList = ref<MsgPushLogDto[]>([]);
