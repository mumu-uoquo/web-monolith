          ...
          <el-main>
            <el-descriptions label-width="100px" :column="3" border>
              ...
              <el-descriptions-item label="消息内容" label-align="right" :span="3">
                <el-text>
                  <div v-html="infoFormData.messageContent"></div>
                </el-text>
              </el-descriptions-item>
...
import SendMsgLog from "./SendMsgLog.vue";
import SendMsgReceiver from "./SendMsgReceiver.vue";
...
/* ***************************** 表单操作 ********************************* */
// 扩展部分属性
interface InfoFormParam extends MsgInfoDto {
  // 预留扩展
  extends?: string;
}
// 初始表单数据
...
const refreshData = (messageId: string) => {
  ...
  MessageAPI.getMessage({ id: messageId }).then((data) => {
    Object.assign(infoFormData.value, data);
  });
};
...
async function handleDeleteReceiver4Message(row: MsgReceiverDto) {
  await MessageAPI.deleteReceiver4Message({ messageId: row.messageId, receiverId: row.receiverId });
...
async function handleListReceiver4Message(param: MsgInfoListParam) {
  pageResult4Receiver.value = await MessageAPI.listReceiverByMessageId({
...
async function handleListLogs4Message(param: MsgInfoListParam) {
  pageResult4Logs.value = await MessageAPI.listPushLogByMessageId({
    pageNum: param.pageNum,
    pageSize: param.pageSize,
    messageId: param.messageId,
  });
