    ...
    <div class="search-bar">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true" label-width="80px">
        ...
        <el-form-item prop="token" label="会话标识">
          <el-input v-model="queryParams.token" placeholder="精确匹配" clearable />
        </el-form-item>
...
import { useDictStore, useUserStore, useReturnCodeStore } from "@/stores";
import LogsAPI, { type BizEventRecordSearchParam, type BizEventRecordDto } from "@/api/logs";
...
async function handleQuery() {
  ...
  if (!queryParams.operatorId) {
    ...
    queryParams.operatorId = userInfo.id;
  }
  LogsAPI.logsByOperationList(queryParams)
    .then((data) => {
...
defineOptions({ name: "logs-operation", inheritAttrs: false });
